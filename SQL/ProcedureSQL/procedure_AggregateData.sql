...
BEGIN
    --(Aggregate) 11.2. Truncate bảng lottery_result_aggregate trong db data_aggregate
		TRUNCATE TABLE data_aggregate.lottery_result_aggregate;
    --(Aggregate) 11.3. Tạo bảng tạm temp_lottery_result_fact
    ...
    SELECT
        ...
        prize.name AS prize,
        lr.winning_number
    --(Aggregate) 11.4. Bảng lottery_result_fact left join trường region_key , province_key, prize_key, date_key với bảng region_dim, province_dim, prize_dim, date_dim để lấy các giá trị tương ứng của db warehouse, giới hạn dữ liệu ngày (date) < 30, insert các giá trị vào bảng tạm temp_lottery_result_fact
    FROM
    ...
    --(Aggregate) 11.5. Insert tất cả dữ liệu từ bảng tạm temp_lottery_result_fact sang bảng lottery_result_aggregate
    ...
    --(Aggregate) 11.6. Xóa bảng tạm temp_lottery_result_fact
